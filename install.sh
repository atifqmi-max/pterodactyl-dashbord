...
show_banner() {
  ...
  echo -e "${BOLD}${PURPLE}"
  if command -v figlet &>/dev/null; then
    figlet -w "$(tput cols 2>/dev/null || echo 120)" -f big "Pterodactyl" | while IFS= read -r line; do center "$line"; done
    figlet -w "$(tput cols 2>/dev/null || echo 120)" -f big "Dashboard" | while IFS= read -r line; do center "$line"; done
  else
    center "=============================="
...
random_string() { tr -dc 'A-Za-z0-9' </dev/urandom | head -c "${1:-24}"; }
...
install_dashboard() {
  ...
  apt-get install -y curl git build-essential nginx mysql-server ufw &>/dev/null
