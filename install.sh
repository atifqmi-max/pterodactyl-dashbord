...
# Auto-adds swap if RAM is low - MySQL commonly fails to start on tiny VPS without this
ensure_swap() {
  local total_mem
  ...
  if [ "$total_mem" -lt 1500 ] && [ "$swap_total" -lt 1024 ]; then
    echo -e "${YELLOW}Low RAM detected (${total_mem}MB). Creating a 2GB swap file...${NC}"
    if [ ! -f /swapfile ]; then
      fallocate -l 2G /swapfile 2>/dev/null || dd if=/dev/zero of=/swapfile bs=1M count=2048 &>/dev/null
      chmod 600 /swapfile
...
# Fully installs, starts, and self-heals MySQL. No manual steps required.
ensure_mysql_running() {
  ...
  # Self-repair: if MySQL still refuses to start, wipe and do a clean reinstall automatically
  if ! mysqladmin ping &>/dev/null; then
    echo -e "${YELLOW}MySQL not responding — auto-repairing (clean reinstall)...${NC}"
    systemctl stop mysql &>/dev/null || systemctl stop mysqld &>/dev/null || true
    apt-get purge -y mysql-server mysql-server-* mysql-client mysql-client-* mysql-common mysql-server-core-* mysql-client-core-* &>/tmp/mysql_purge.log
    rm -rf /etc/mysql /var/lib/mysql
    apt-get autoremove -y &>>/tmp/mysql_purge.log
    apt-get update -y &>>/tmp/mysql_purge.log
    apt-get install -y mysql-server &>/tmp/mysql_reinstall.log
    ...
    until mysqladmin ping &>/dev/null; do
      tries=$((tries+1))
      if [ $tries -gt 15 ]; then
        echo -e "${RED}MySQL still failed to start after auto-repair.${NC}"
        echo -e "${RED}Showing last log lines for diagnosis:${NC}"
        journalctl -xeu mysql.service --no-pager -n 30
        exit 1
      fi
      sleep 2
    done
